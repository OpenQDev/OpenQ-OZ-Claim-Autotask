...
const parsePRText = require('../lib/parsePRText');
		...
		try {
			...
			for (let timelineItem of timelineItems) {
				if (isPullRequest(timelineItem)) {
					...
					const { merged, prAuthor, baseRepositoryOwner, bodyText, bodyEdits, pullRequestCreatedAt, mergedAt } = extractPullRequestAttributes(pullRequest);

					let bodyTextAtMergeTime = contentAtMergeTime(pullRequestCreatedAt, mergedAt, bodyText, bodyEdits);
					console.log(bodyEdits);
					console.log(pullRequestCreatedAt);
					...
					let prBodyContainedIssueCloserCommentAtMergeTime = bodyTextAtMergeTime.includes(`Closes #${issueNumber}`);

					if (viewerIsPRAuthor && prIsMerged && baseRepositoryOwnerIsIssueOwner && prBodyContainedIssueCloserCommentAtMergeTime) {
