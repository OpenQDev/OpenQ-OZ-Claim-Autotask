...
const checkWithdrawalEligibility = async (issueUrl, oauthToken) => {
	return new Promise(async (resolve, reject) => {
		...
		try {
			let { data } = await getIssueClosedEvents(issueId, oauthToken);
			...
			const author = closer.author.login;
			const prUrl = closer.url;

			if (closer == viewer) {
				return resolve(true);
			} else {
				return reject(ISSUE_NOT_CLOSED_BY_USER({ issueUrl, viewer, closer, prUrl }));
			}
		} catch (error) {
			if (error.response && error.response.status == 401) {
				return reject(GITHUB_OAUTH_TOKEN_LACKS_PRIVILEGES({ issueId }));
