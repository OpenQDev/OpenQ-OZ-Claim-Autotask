...
const contentAtMergeTime = (pullRequestCreatedAt, mergedAt, bodyText, userContentEdits) => {
	...
	if (userContentEdits.length == 0) {
		...
		for (let userContentEdit of userContentEdits) {
			...
			if (editIsInRangeBetweenCreationAndMerge(pullRequestCreatedAtDate, mergedAtDate, editedAtDate)) {
				inRangeEdits.push(userContentEdit);
			}
		}

		// Choose latest one in that range
		const latestEditBeforeMerge = inRangeEdits.shift().diff;

		return latestEditBeforeMerge;
