	...
	describe('Correctly verifies withdrawal eligibility', () => {
		describe('Ineligible Responses', () => {
			it('should resolve with NO_PULL_REQUESTS_REFERENCE_ISSUE if none reference the issue', async () => {
				const issuePrData = { data: { viewer: { login: 'FlacoJones' }, resource: { id: 'I_kwDOGWnnz85GZu4Y', number: 123, repository: { owner: { login: 'OpenQDev' } }, timelineItems: { edges: [] } } } };
		...
		describe('Eligible Response', () => {
			it.only('should resolve to with canWithdraw: true if eligible pull request is connected in BODY', async () => {
				...
				await expect(checkWithdrawalEligibility(issueUrl, oauthToken)).resolves.toEqual({ 'canWithdraw': true, type: 'SUCCESS', issueId: 'I_kwDOGWnnz85GjwA1', errorMessage: null });
			});
